...
For #SAT (counting satisfying assignments), we know it's **#P-complete**. This means it's at least as hard as any NP problem, probably harder. Toda's Theorem tells us the entire polynomial hierarchy collapses to $\text{P}^{\\#\text{P}}$ (if you can count, you can solve anything in PH). Counting is powerful.
...
When I compiled CNF formulas to d-DNNF (decision diagrams for model counting), I was measuring **circuit complexity**: how many logical gates (AND, OR, NOT) does it take to represent this Boolean function?
...
Here's the interesting part: we can't prove *any* superlinear lower bounds for explicit functions. Shannon proved in 1949 that most functions need exponential circuits (by counting), but we can't point to a specific function and prove it needs more than $O(n)$ gates. The best result is Ryan Williams' 2011 breakthrough: NEXP is not in ACC$^0$ (circuits with modular counting gates).
...
**Query complexity** (also called decision tree complexity) asks a purer question: ignoring computation entirely, how many input bits must you examine to compute the answer?
...
Example: the OR function (is there a 1 in the input?). Deterministically, you need $D(\text{OR}) = n$ queries because the 1 might be in the last position you check. But Grover's algorithm gives $Q(\text{OR}) = O(\sqrt{n})$ with a quantum computer. This quadratic speedup is the canonical quantum search advantage.
...
One of my favorite results in query complexity is Hao Huang's 2019 proof of the **Sensitivity Conjecture**. Sensitivity $s(f)$ measures how many bits you can flip in an input to change the output. Block sensitivity $bs(f)$ is similar but for blocks of bits. For 25 years, people wondered if $bs(f)$ was polynomially bounded by $s(f)$. Huang proved $bs(f) \leq s(f)^4$ with a two-page argument using spectral graph theory and the Cauchy Interlace Theorem.

Two pages. 25-year-old problem. That's the kind of math that makes you spill coffee.

## Proof Complexity

Here's where things get weird. **Proof complexity** asks: how many steps does it take to *prove* something is unsatisfiable?

Most modern SAT solvers use CDCL (Conflict-Driven Clause Learning), which is essentially doing **Resolution**: a proof system where you derive new clauses from existing ones.
...
Haken's 1985 result: the Pigeonhole Principle ($n+1$ pigeons in $n$ holes) requires exponential-size Resolution proofs. This means CDCL solvers *must* take exponential time on these formulas. The hardness isn't in the problem (easy to state), it's in the *structure of reasoning required*.
...
Ben-Sasson and Wigderson formalized this with the **width-size tradeoff**: if a Resolution proof has small width (short clauses), then it's short. But if every proof requires wide clauses (mentioning many variables), then every proof is long. Random 3-CNF at the critical density has high proof width, hence high proof complexity, hence hard for CDCL.
...
And here's the kicker: Cook-Reckhow (1979) proved that if there exists a proof system where every tautology has polynomial-size proofs, then $\text{NP} = \text{coNP}$. So proving lower bounds for increasingly strong proof systems is a path toward separating complexity classes. We've done it for Resolution, Cutting Planes, bounded-depth Frege. For general Frege (standard propositional logic)? Still open.

## Kolmogorov Complexity

Okay, curveball time. **Kolmogorov complexity** measures something completely different: information content, independent of computation time.
...
Liu and Pass (2020) proved: **one-way functions exist if and only if Kt complexity is hard to compute on average**. This connects cryptographic hardness to meta-complexity (the complexity of computing complexity).
...
**Communication complexity**: Alice has $x$, Bob has $y$, they want to compute $f(x, y)$. They exchange bits. The **communication complexity** $D(f)$ is the minimum number of bits needed in the worst case.
...
Why does this matter? Because communication lower bounds imply circuit lower bounds. The **Karchmer-Wigderson relation** connects circuit depth to communication complexity: the depth of a circuit computing $f$ equals the communication complexity of a certain two-party game about $f$. This is how we prove AC$^0$ lower bounds.
...
Here's where things get elegant. **Algebraic complexity** (also called arithmetic complexity) studies computation over fields instead of bits. Instead of Boolean circuits with AND/OR/NOT gates, we have arithmetic circuits with $+, -, \times$ gates over fields like $\mathbb{R}$ or $\mathbb{C}$.

Leslie Valiant defined two classes to mirror P and NP:
...
Why is this "cleaner" than Boolean complexity? Because the algebraic setting has more structure. We can use tools from algebraic geometry, representation theory, and commutative algebra. The **Geometric Complexity Theory** (GCT) program, launched by Mulmuley and Sohoni, tries to prove VP $\neq$ VNP using representation-theoretic obstructions.
...
Completely different angle. **Descriptive complexity** measures problems by the *logical formulas* needed to describe them.

Fagin's Theorem (1974): **NP = Existential Second-Order Logic**. A problem is in NP iff it's expressible as "there exists a relation $R$ such that $\phi(R)$ holds" for some first-order $\phi$.
...
- **P** = First-Order + Least Fixed Point (on ordered structures)
...
**Parameterized complexity** refines P vs NP by acknowledging inputs have structure.
...
So here is the thing: Dr. Vardi is my undergraduate advisor, and it so happens that I had to read one of papers to lecture for a class he teaches where he talks about another type of complexity. In his 1982 paper "The Complexity of Relational Query Languages," he made a distinction that should have been obvious but somehow wasn't standard at the time.

Vardi asked: when we talk about the complexity of a query language, what are we actually measuring?
...
**Note**: If you want #SAT details, wait for the paper. If you think topological complexity doesn't belong here (I know, I know), email me. If you've been nerd-sniped and want to dive deeper, the references in the documents I used are gold.
